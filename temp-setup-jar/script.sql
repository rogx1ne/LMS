...
-- Safe setup: do not drop existing schemas or grant DBA.
BEGIN
...
-- Cleanup existing LMS objects inside the schema so the script can be rerun safely on Oracle 10g.
BEGIN EXECUTE IMMEDIATE 'DROP TABLE TBL_ISSUE CASCADE CONSTRAINTS'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -942 THEN RAISE; END IF; END;
/
BEGIN EXECUTE IMMEDIATE 'DROP TABLE TBL_BOOK_ALERT_LOG CASCADE CONSTRAINTS'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -942 THEN RAISE; END IF; END;
/
BEGIN EXECUTE IMMEDIATE 'DROP TABLE TBL_ORDER_DETAILS CASCADE CONSTRAINTS'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -942 THEN RAISE; END IF; END;
/
BEGIN EXECUTE IMMEDIATE 'DROP TABLE TBL_ORDER_HEADER CASCADE CONSTRAINTS'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -942 THEN RAISE; END IF; END;
/
BEGIN EXECUTE IMMEDIATE 'DROP TABLE TBL_BILL CASCADE CONSTRAINTS'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -942 THEN RAISE; END IF; END;
/
BEGIN EXECUTE IMMEDIATE 'DROP TABLE TBL_BOOK_STOCK CASCADE CONSTRAINTS'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -942 THEN RAISE; END IF; END;
/
BEGIN EXECUTE IMMEDIATE 'DROP TABLE TBL_BOOK_INFORMATION CASCADE CONSTRAINTS'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -942 THEN RAISE; END IF; END;
/
BEGIN EXECUTE IMMEDIATE 'DROP TABLE TBL_BOOK_CATALOG CASCADE CONSTRAINTS'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -942 THEN RAISE; END IF; END;
/
BEGIN EXECUTE IMMEDIATE 'DROP TABLE TBL_STUDENT CASCADE CONSTRAINTS'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -942 THEN RAISE; END IF; END;
/
BEGIN EXECUTE IMMEDIATE 'DROP TABLE TBL_SELLER CASCADE CONSTRAINTS'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -942 THEN RAISE; END IF; END;
/
BEGIN EXECUTE IMMEDIATE 'DROP TABLE TBL_AUDIT_LOG CASCADE CONSTRAINTS'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -942 THEN RAISE; END IF; END;
/
BEGIN EXECUTE IMMEDIATE 'DROP TABLE TBL_ID_COUNTER CASCADE CONSTRAINTS'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -942 THEN RAISE; END IF; END;
/
BEGIN EXECUTE IMMEDIATE 'DROP TABLE TBL_CREDENTIALS CASCADE CONSTRAINTS'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -942 THEN RAISE; END IF; END;
/

BEGIN EXECUTE IMMEDIATE 'DROP SEQUENCE SEQ_AUDIT_LOG'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -2289 THEN RAISE; END IF; END;
/
BEGIN EXECUTE IMMEDIATE 'DROP SEQUENCE SEQ_BOOK_ALERT_LOG'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -2289 THEN RAISE; END IF; END;
/
...
CREATE TABLE TBL_CREDENTIALS (
    ...
    NAME      VARCHAR2(50) NOT NULL, -- NAME OF THE USER
    PSWD	VARCHAR2(255) NOT NULL, -- HASHED PASSWORD
    EMAIL     VARCHAR2(80) NOT NULL,    -- EMAIL OF THE USER
...
CREATE TABLE TBL_ID_COUNTER (
    COUNTER_KEY VARCHAR2(50) CONSTRAINT ID_COUNTER_PK PRIMARY KEY,
    NEXT_VAL    NUMBER(10) NOT NULL
);

CREATE TABLE TBL_AUDIT_LOG (
    LOG_ID              NUMBER CONSTRAINT AUDIT_PK PRIMARY KEY,
...
BEFORE INSERT ON TBL_AUDIT_LOG
FOR EACH ROW
BEGIN
...
CREATE TABLE TBL_BOOK_INFORMATION (
    ACCESS_NO   CHAR(6) CONSTRAINT BK_INFO_PK PRIMARY KEY, -- ACCESSION NUMBER OF THE BOOK (6-digit numeric)
    AUTHOR_NAME VARCHAR2(50) NOT NULL, -- AUTHOR NAME OF THE BOOK
    BK_TITLE    VARCHAR2(100) NOT NULL, -- TITLE OF THE BOOK
    ...
    TAGS        VARCHAR2(500), -- SEARCHABLE TAGS (COMMAS SEPARATED)
...
CREATE SEQUENCE SEQ_BOOK_ALERT_LOG START WITH 1 INCREMENT BY 1;

CREATE OR REPLACE TRIGGER TRG_BOOK_ALERT_LOG
BEFORE INSERT ON TBL_BOOK_ALERT_LOG
FOR EACH ROW
BEGIN
    IF :NEW.ALERT_ID IS NULL THEN
        SELECT SEQ_BOOK_ALERT_LOG.NEXTVAL INTO :NEW.ALERT_ID FROM DUAL;
    END IF;
END;
/
...
CREATE TABLE TBL_STUDENT (
    CARD_ID   CHAR(9) CONSTRAINT STUD_PK PRIMARY KEY, -- LIBRARY CARD ID OF THE STUDENT (PPU-YYxxx)
    ROLL      NUMBER(5) NOT NULL, -- ROLL NUMBER OF THE STUDENT
    ...
    CONSTRAINT UK_STUD_ROLL_COURSE_SESS UNIQUE (ROLL, COURSE, ACAD_SESSION),
...
CREATE TABLE TBL_BILL (
    ...
    CONSTRAINT BILL_PK PRIMARY KEY (B_ID, TTL, AUTHOR), -- PRIMARY KEY CONSTRAINT
    CONSTRAINT FK_BILL_SELLER FOREIGN KEY (S_ID) REFERENCES TBL_SELLER(S_ID) -- FOREIGN KEY CONSTRAINT
);
...
CREATE TABLE TBL_ISSUE (
    ...
    BORROWER_TYPE VARCHAR2(10) DEFAULT 'STUDENT' NOT NULL, -- STUDENT / FACULTY
    CARD_ID       CHAR(9), -- STUDENT CARD ID
    FACULTY_NAME  VARCHAR2(80), -- FACULTY BORROWER NAME
    FACULTY_CONTACT VARCHAR2(15), -- FACULTY CONTACT NO
    ACCESSION_NO  CHAR(6) NOT NULL, -- BOOK ACCESSION NO (Matches TBL_BOOK_INFORMATION.ACCESS_NO)
    ...
    FINE          NUMBER(8,2) DEFAULT 0 NOT NULL, -- RETURN FINE
    ISSUED_BY     VARCHAR2(20) NOT NULL, -- LIBRARIAN USER ID
    STATUS        VARCHAR2(10) DEFAULT 'ISSUED' NOT NULL, -- ISSUED / RETURNED
    CONSTRAINT CHK_ISSUE_BORROWER_TYPE CHECK (BORROWER_TYPE IN ('STUDENT','FACULTY')),
    ...
    CONSTRAINT CHK_ISSUE_BORROWER CHECK (
        (BORROWER_TYPE = 'STUDENT' AND CARD_ID IS NOT NULL AND FACULTY_NAME IS NULL AND FACULTY_CONTACT IS NULL) OR
        (BORROWER_TYPE = 'FACULTY' AND CARD_ID IS NULL AND FACULTY_NAME IS NOT NULL AND FACULTY_CONTACT IS NOT NULL)
    ),
...
CREATE INDEX IDX_ISSUE_BORROWER_TYPE ON TBL_ISSUE(BORROWER_TYPE, STATUS);
CREATE INDEX IDX_ISSUE_FACULTY_NAME ON TBL_ISSUE(FACULTY_NAME, STATUS);
CREATE INDEX IDX_ISSUE_ACC_STATUS ON TBL_ISSUE(ACCESSION_NO, STATUS);

INSERT INTO TBL_CREDENTIALS
VALUES('ADMIN','ADMINISTRATOR','SHA256$835d6dc88b708bc646d6db82c853ef4182fabbd4a8de59c213f2b5ab3ae7d9be','admin@lms.in','1234567890','ADMIN','ACTIVE');
INSERT INTO TBL_CREDENTIALS
VALUES('LIB01','LIBRARIAN','SHA256$163e3b05d7d2aa6159c0a41c33afcdb49e77c55d94a6699c7e1f09a19dc95ad5','lib01@lms.in','1234567890','LIBRARIAN','ACTIVE');
