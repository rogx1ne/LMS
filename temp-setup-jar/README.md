...
### Prerequisites
- **Java:** JDK 8 or higher (recommend Java 8 for compatibility)
- **Oracle Database:** Oracle 10g XE or higher
  - If using Podman: Container running with port 1521 exposed (e.g., `podman start oracle10g`)
- **JDBC Driver:** ojdbc6.jar (already included in `lib/`)

### Quick Start (with Podman Oracle)
```bash
...
- ✓ Verifies Podman Oracle container is running
...
podman cp script.sql oracle10g:/
podman exec oracle10g sqlplus system/oracle@xe @/script.sql

# Load demo data (optional)
podman cp dummy.sql oracle10g:/
podman exec oracle10g sqlplus PRJ2531H/PRJ2531H@xe @/dummy.sql
```

#### 2) Connection Configuration
The application reads from environment variables (or falls back to hardcoded values):
```bash
...
Or manually edit `src/com/library/database/DBConnection.java` to update default credentials.
...
**Linux:**
```bash
./run.sh              # Uses environment variables if set
./run-with-env.sh     # Recommended - sets everything automatically
```
...
After loading `dummy.sql`, use these credentials to test:
...
**Note:** Passwords are hashed with SHA-256. Default passwords match the User ID.
